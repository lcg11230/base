...
import com.lcg.base.surpport.entity.EntityField;
...
public class FieldHelper {
    ...
    public static List<EntityField> getAll(Class<?> entityClass) {
        ...
        Iterator var5;
        EntityField field;
        for (var5 = fields.iterator(); var5.hasNext(); all.add(field)) {
            field = (EntityField) var5.next();
            Iterator var7 = properties.iterator();

            while (var7.hasNext()) {
                EntityField field1 = (EntityField) var7.next();
                if (!usedSet.contains(field1) && field.getName().equals(field1.getName())) {
                    field.copyFromPropertyDescriptor(field1);
                    usedSet.add(field1);
                    break;
                }
            }
    ...
    static class Jdk6_7FieldHelper implements FieldHelper.IFieldHelper {
        ...
        public List<EntityField> getProperties(Class<?> entityClass) {
            ...
            try {
                ...
                throw new MapperException(var11);
            }
